window.addEventListener('libs-ready', () => {
    ...
    function applyFiltersAndRender() {
        // Transformar los datos si es necesario (ej: si son solo strings de IDs)
        ...
        // La lógica de filtros solo se aplicará si hay filtros activos
        ...
        for (const trait in appState.activeFilters) {
            ...
            filteredData = filteredData.filter(nft =>
                // Importante: Verificar que el NFT tenga atributos antes de filtrar
                nft.attributes && nft.attributes.some(attr => attr.trait_type === trait && appState.activeFilters[trait].includes(attr.value))
            );
        }

        // Renderizar el grid con los datos filtrados y formateados
        UIElements.collectionsGrid.innerHTML = '';
        ...
        for (const nft of filteredData) {
            ...
            // La función renderNftCard ya es robusta y usará valores por defecto
            // si no encuentra 'name', 'attributes', etc.
    ...
    function renderFilters(collectionData) {
        ...
        const traits = {}; // Usaremos un objeto para agrupar valores por tipo de rasgo
    ...
    // 2. Construir el HTML de los filtros
        let filtersHTML = '<h3>Filters</h3>';
        for (const trait in traits) {
            filtersHTML += `<fieldset class="filter-group"><legend>${trait}</legend>`;
        ...
        filtersHTML += `
            <fieldset class="filter-group"><legend>Status</legend>
                <div class="filter-option">
                    ...
                    <label for="filter-status-listed">Solo en Venta</label>
                </div>
            </fieldset>
        `;
    ...
    function renderNftCard(data, gridElement, context) {
        ...
        if (attributes && attributes.length > 0) {
            ...
            attributes.forEach(attr => {
                attributesHTML += `<div class="attribute-tag"><strong>${attr.trait_type}:</strong> ${attr.value}</div>`;
            });
            attributesHTML += '</div>';
        }
        ...
        card.innerHTML = `
            <div class="card-selection">
                <input type="checkbox" class="nft-checkbox" data-file-id="${file_id}" ${isSelected ? 'checked' : ''}>
            </div>
        ...
        checkbox.addEventListener('change', () => handleItemSelection(file_id));
    ...
    function handleApiError(error) {
        if (error.message && error.message.includes("Server-side check failed")) {
    ...
    function handleSelectUnlisted() {
        if (!appState.wallet || appState.myNfts.length === 0) return;

        
        const unlistedNfts = appState.myNfts.filter(nft => !nft.price || parseFloat(nft.price) <= 0);

        
        const nftsToSelect = unlistedNfts.slice(0, 100);

        
        const idsToSelect = nftsToSelect.map(nft => nft.file_id);

        
        appState.selectedItems = idsToSelect;

        
        renderMyWalletView();
        renderBulkActionBar(); // Y muestra la barra de acciones
    }
